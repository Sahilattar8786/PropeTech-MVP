...
function place(facts: PropertyFacts): string | null {
  return facts.location?.locality ?? facts.location?.city ?? null;
}

/** "3 BHK Apartment in Whitefield" */
export function basicTitle(facts: PropertyFacts): string {
  const where = place(facts);
  return `${configuration(facts)}${where ? ` in ${where}` : ""}`;
}

/** Title with a descriptor grounded in the facts (furnishing or price tier). */
export function enrichedTitle(facts: PropertyFacts): string {
  let descriptor = "";
  ...
  return [descriptor, basicTitle(facts)].filter(Boolean).join(" ");
}
...
export function templateDescription(facts: PropertyFacts): string {
  const sentences: string[] = [];
  const config = configuration(facts);
  const loc = [facts.location?.locality, facts.location?.city].filter(Boolean).join(", ");
  ...
  sentences.push(`${article(subject)} ${subject}${loc ? ` in ${loc}` : ""}, ${offer}${price ? ` at ${price}` : ""}.`);
  ...
  if (details.length) sentences.push(`The property ${listOf(details)}.`);
...
export function templateCopy(facts: PropertyFacts, opts: { enriched: boolean }): PropertyCopy {
  const title = opts.enriched ? enrichedTitle(facts) : basicTitle(facts);
  const description = opts.enriched ? templateDescription(facts) : null;
  return {
    title,
    description,
